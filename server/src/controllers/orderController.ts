...
const attachUnreadCounts = async (orders: any[], userRole: string) => {
  if (userRole !== 'elderly' && userRole !== 'volunteer') {
      return orders.map((order) => ({ ...order.get({ plain: true }), unreadCount: 0 }));
  }

  const unreadField = userRole === 'elderly' ? 'readByElderly' : 'readByVolunteer';
...
export const getOrders = async (req: AuthRequest, res: Response) => {
  try {
    ...
    const ordersWithUnread = await attachUnreadCounts(orders || [], user.role);
...
export const rejectOrderCompletion = async (req: AuthRequest, res: Response) => {
    try {
        const user = req.user;
        const { id } = req.params;
        ...
        if (order.elderlyId !== user.id && user.role !== 'admin') {
            return res.status(403).json({ message: 'Not authorized to reject this completion submission' });
        }
        ...
        order.completionRejectionReason = String(reason).trim();
        await order.save();
        ...
        res.json({ message: 'Completion submission rejected', order });
    } catch (error) {
