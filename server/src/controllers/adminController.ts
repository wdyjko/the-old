...
export const auditOrder = async (req: AuthRequest, res: Response) => {
    try {
        ...
        const { id } = req.params;
        const { status, pointsReward } = req.body; // 'pending', 'rejected', 'submitted'
        ...
        const isCompletionDispute = Boolean(order.volunteerId && order.completionRejectionReason);

        if (!isCompletionDispute && status === 'submitted') {
            return res.status(400).json({ message: 'Only disputed completion orders can be returned for family reconfirmation' });
        }
        ...
        if (isCompletionDispute) {
            if (status === 'pending') {
                order.status = 'pending';
                order.volunteerId = null;
                order.completionDescription = null;
                order.completionPhotos = null;
        ...
        const messageMap: Record<string, string> = {
            pending: isCompletionDispute ? 'Order returned to task hall successfully' : 'Order approved successfully',
            submitted: 'Order returned to family for reconfirmation successfully',
            rejected: 'Order rejected successfully',
        };

        res.json({ message: messageMap[status], order });
    } catch (error) {
